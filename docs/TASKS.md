...
v0.19.0d Engine Privacy Boundary Hardening / Release Notes delivered — `src/lib/parent-review-session-history.ts` `FORBIDDEN_PARENT_FIELDS` extended with 16 engine / KataGo keys (`topMoves | visits | scoreLead | winrate | playouts | engineHint | engineReview | engineSignal | engineAssisted | engineConfidence | agreedWithAuthoredAnswer | authoredAnswerRank | attemptedMoveRank | engineLatency | engineDiagnostics | lastAnalysis`); 7 new boundary regression tests pin the engine field extensions and confirm no false-positive on legitimate aggregate fields; `docs/AI_ENGINE_BOUNDARY_QA_v0.19.md` cross-surface inspection confirms no engine field can reach `/report`, `/wrong-book`, `/dev/session-summary`, or `localStorage`; `docs/RELEASE_NOTES_v0.19.md` and `docs/QA_CHECKLIST_v0.19.md` close the v0.19 series. v0.19 series complete (638 tests across 31 files). Next: v0.20.0a — Next-Phase Plan (planning-only).

v0.20.0a Next-Phase Plan delivered — `docs/NEXT_PHASE_PLAN_v0.20.md` re-anchors v0.19 helpers and v0.19.0d privacy boundary, evaluates 4 candidate directions, and scopes the v0.20 series to **gated consumer wiring for the v0.19 helpers + a content-only endgame/mixed expansion pilot**. Specifically: v0.20.0b wires `explainChildEngine()` into `ProblemPlayer` on multi-step wrong attempts behind a new `CHILD_ENGINE_EXPLAIN` feature flag (env → runtime → default, off by default); v0.20.0c wires `buildEngineHint()` into `ProblemPlayer` to project a single `Highlight` of type `"hint"` after the first wrong attempt behind the existing `ENGINE_HINT_PROJECTION` flag; v0.20.0d adds 4 endgame + 5 mixed human-reviewed problems (Pack B pilot, content-only). Infrastructure (4.3) and parent review surface (4.4) are deferred to v0.21+. v0.1 boundary respected; planning-only slice, no code change. Next: v0.20.0b — `explainChildEngine()` consumer wiring for multi-step wrong attempts.

v0.20.0b `explainChildEngine()` Consumer Wiring for Multi-Step delivered — `src/lib/child-engine-explain.ts` extended with `CHILD_ENGINE_EXPLAIN` feature flag contract (env → runtime → default, env wins, off by default) and `shouldUseChildEngineExplain(isMultiStep)` consumer gate; `src/components/problem/ProblemPlayer.tsx` adds `handleShowChildCoach()` (pure local `explainChildEngine()` path with an **honest low-confidence signal** so the helper's `source` gate renders `rule-template`, `coachRequestId` stale guard, `validateChildEngineExplain` defense-in-depth) and routes the "请老师帮忙" button through it instead of `handleShowCoach` when the gate is on. Off-state: single-step + multi-step + flag-off both byte-identical to v0.19. **16 new tests** (8 flag contract + 8 wiring + privacy regression in `ProblemPlayer.multi-step.test.tsx` covering single-step skip, multi-step flag off, multi-step flag on with non-empty message, **source = rule-template regression** proving no `engine-assisted` caption is rendered without a real engine review, validator contract, single-fire, try-again reset, 16-key engine-privacy regression). v0.19.0d `FORBIDDEN_PARENT_FIELDS` privacy boundary preserved end-to-end; the v0.20.0b path does **not** synthesize `engine-assisted` on a local-only path. Next: v0.20.0c — `buildEngineHint()` consumer wiring for single `Highlight` projection.

v0.20.0c `buildEngineHint()` Consumer Wiring for Single Highlight delivered — `src/components/problem/ProblemPlayer.tsx` adds an `engineHint` state slot, a projection effect that fires `buildEngineHint()` on the first wrong attempt of a problem (single-step or multi-step) when the `ENGINE_HINT_PROJECTION` flag is on, and merges the resulting `point` into the existing `GoBoard` `highlights` prop as a single `hint` highlight. Off-state: flag off / non-first attempt / no-hint from helper all resolve to either not firing or producing a `null` state, so the rendered highlights are byte-identical to v0.19. Reset hooks: `try-again`, `problem.id` change, and the per-attempt `currentWrongAttempts` guard. **7 new tests** in `ProblemPlayer.multi-step.test.tsx` cover flag off (single-step + multi-step), flag on (single-step + multi-step top-moves-undefined renders no hint highlight), try-again reset, problem-change clear, and highlight-type allowlist regression (only `wrong` / `correct` / `hint` are ever rendered). v0.19.0d `FORBIDDEN_PARENT_FIELDS` privacy boundary preserved end-to-end: the path passes `topMoves: undefined` and an honest low-confidence signal so the helper's `low-confidence` gate fires and returns `no-hint` until a real engine payload is provided by a future slice. Next: v0.20.0d — content-only Pack B pilot (4 endgame + 5 mixed problems).
...
v0.21.0b Pack B Chapter/Daily-Practice Wiring delivered — `src/lib/chapters.ts` extended with (1) a new `endgame-5` level containing the v0.20.0d Pack B endgame problems (END-013..016, level 3-5), preserving the existing endgame-1..endgame-4 level ids for backward compatibility with saved progress and chapter-by-id links; (2) a new `mixed` chapter "综合擂台" with 4 levels (mixed-1..mixed-4) covering all 8 mixed problems (the 3 pre-existing v0.4.0b MIX-001..003 plus the 5 v0.20.0d Pack B MIX-004..008). `getAllProblemIds()` now covers 99 problems (was 87); the daily-practice pool automatically samples the newly-wired problems with no selection-algorithm change. **15 new tests** in `src/__tests__/chapters.test.ts` (chapter structure, level uniqueness, every problemId resolves, Pack B endgame wired in endgame-5, new mixed chapter with 8 problems, categoryLabels mixed entry, daily pool scope assertion, out-of-scope assertion pinning END-011/012/CAP-022/CC-018 as deferred to a later slice, Pack B chapter placement, Pack B exact-once duplicate protection, global no-duplicate regression). No algorithm change, no UI change, no route / persistence / telemetry change, v0.1 boundary preserved. v0.21.0b is the conservative low-risk Pack B wiring slice; END-011/012/CAP-022/CC-018 remain in problems.json but unwired (deferred to a follow-up slice, not in v0.21.0b scope). Next: v0.21.0c — v0.21 release notes / QA checklist / stabilization.
...
v0.22.0b Wire Remaining 4 Unwired v0.7.0b Problems delivered — `src/lib/chapters.ts` extended with 3 new level nodes: `endgame-6` (END-011, END-012), `capture-13` (CAP-022), `connect-cut-9` (CC-018). `getAllProblemIds()` now covers 103 problem IDs (was 99). **10 new v0.22.0b tests added, 1 obsolete v0.21.0b negative test removed** (net +9, 691 total across 32 files) in `src/__tests__/chapters.test.ts` (count assertion 99→103, each target ID in correct chapter, new level nodes exist, exact-once duplicate protection, global no-duplicate regression, existing v0.21.0b Pack B wiring preserved). No problem data change, no algorithm change, no UI change, no route / persistence / telemetry change. v0.1 boundary preserved. Next: v0.22.0c — v0.22 release notes / QA checklist / stabilization.
...
73. v0.19.0d Engine Privacy Boundary Hardening / Release Notes completed — `src/lib/parent-review-session-history.ts` `FORBIDDEN_PARENT_FIELDS` extended with 16 engine / KataGo keys (`topMoves | visits | scoreLead | winrate | playouts | engineHint | engineReview | engineSignal | engineAssisted | engineConfidence | agreedWithAuthoredAnswer | authoredAnswerRank | attemptedMoveRank | engineLatency | engineDiagnostics | lastAnalysis`); 7 new boundary regression tests pin the extensions and confirm no false-positive on legitimate aggregate fields; cross-surface inspection confirms no engine field can reach `/report`, `/wrong-book`, `/dev/session-summary`, or `localStorage`; `docs/AI_ENGINE_BOUNDARY_QA_v0.19.md`, `docs/RELEASE_NOTES_v0.19.md`, and `docs/QA_CHECKLIST_v0.19.md` close the v0.19 series (638 tests across 31 files). Next: v0.20.0a — Next-Phase Plan (planning-only).
74. v0.20.0a Next-Phase Plan completed — `docs/NEXT_PHASE_PLAN_v0.20.md` re-anchors v0.19 helpers and v0.19.0d privacy boundary, evaluates 4 candidate directions, scopes the v0.20 series to gated consumer wiring for v0.19 helpers (0b / 0c) + a content-only endgame/mixed expansion pilot (0d). Infrastructure (4.3) and parent review surface (4.4) deferred to v0.21+. v0.1 boundary respected; planning-only slice. Next: v0.20.0b — `explainChildEngine()` consumer wiring for multi-step wrong attempts behind `CHILD_ENGINE_EXPLAIN` flag.
...
83. v0.22.0b Wire Remaining 4 Unwired v0.7.0b Problems completed — `src/lib/chapters.ts` extended with 3 new level nodes: `endgame-6` (END-011, END-012), `capture-13` (CAP-022), `connect-cut-9` (CC-018). `getAllProblemIds()` covers 103 problems (was 99). **10 new v0.22.0b tests added, 1 obsolete v0.21.0b negative test removed** (net +9, 691 total) in `src/__tests__/chapters.test.ts`. No problem data / algorithm / UI / route change. v0.1 boundary preserved. Next: v0.22.0c — v0.22 release notes / QA checklist / stabilization.
84. v0.22.0c v0.22 Stabilization / Release Notes completed — `docs/RELEASE_NOTES_v0.22.md` (v0.22.0a–v0.22.0b) and `docs/QA_CHECKLIST_v0.22.md`. v0.22 series complete (691 tests across 32 files). Next: v0.23.0a — Next-Phase Plan (planning-only).
```
