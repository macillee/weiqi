...
v0.19.0c Wrong-Move Engine Explanation (Multi-Step) Pure Helper delivered — `src/lib/child-engine-explain.ts` exposes pure `explainChildEngine(input)` returning a `LocalReviewResult`-shaped object; `source` is `"engine-assisted"` only when signal agrees with authored answer AND confidence is `medium` or `high` (otherwise `"rule-template"` fallback); rank-aware refinement when both ranks are finite; extended `validateChildEngineExplain()` covers length ≤150, banned phrases (carried from `ai-review.ts` plus engine-specific: 胜率 / winrate / rating / score / 得分 / visits / playouts / etc.), source enum, non-empty concept; **18 new tests** cover: engine-assisted on high/medium confidence, rule-template fallback on low / disagree / malformed signal, all 7 categories, banned-phrase sweep across 4 signal × 7 category combinations, determinism, rank-aware refinement (close-by / far), malformed rank safety (null / undefined / NaN / 0), hint-aware rule-template fallback, multi-step input, unknown category fallback, and the validator (valid / empty / too-long / banned / bad-source). **No consumer wiring; no change to `ProblemPlayer` / `FeedbackDialog` / server action**. Next: v0.19.0d — engine privacy boundary hardening / release notes / QA checklist.
...
72. v0.19.0c Wrong-Move Engine Explanation (Multi-Step) Pure Helper completed — `src/lib/child-engine-explain.ts` (pure `explainChildEngine()` returning a `LocalReviewResult`), `source` is `engine-assisted` only on agree + medium/high confidence, rank-aware refinement when both ranks are finite, extended `validateChildEngineExplain()` for length / banned / source / empty. **18 new tests** cover engine-assisted on high/medium, rule-template fallback on low / disagree / malformed signal, all 7 categories, banned-phrase sweep across 4 signal × 7 category, determinism, rank refinement (close-by / far), malformed rank safety (null / undefined / NaN / 0), hint-aware rule-template fallback, multi-step input, unknown category fallback, and the validator (valid / empty / too-long / banned / bad-source). **No consumer wiring; no `ProblemPlayer` / `FeedbackDialog` / server action change**. Next: v0.19.0d — engine privacy boundary hardening / release notes / QA checklist.
```
