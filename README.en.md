...
- **UX polish**: 9×9 SVG board, Chinese coordinate labels (一–九), celebration star animation, toggleable answer audio, progressive hint cards with deterministic board markers
- **Tests**: `npm run test` — 293 passing across 20 test files
- **Build**: `npm run build` passes; `docker compose up --build` runs the app locally

Next up: **v0.6 stabilization & release notes**.
...
- **Progressive hints**: each hint is a numbered card with a fade-in animation; hints that mention a `(x, y)` coordinate also add a small outlined ring marker on the board (only while no answer is submitted)
