...
可选：在 `.env.local` 中配置 `NEXT_PUBLIC_SUPABASE_URL` 与 `NEXT_PUBLIC_SUPABASE_ANON_KEY` 启用云端账号、孩子档案与进度同步。Docker Compose 会自动从 `.env.local` 或 shell 环境读取这两个变量。**未配置时不影响任何本地功能**。
