...
可选：在 `.env.local` 中配置 `NEXT_PUBLIC_SUPABASE_URL` 与 `NEXT_PUBLIC_SUPABASE_ANON_KEY` 启用云端账号、孩子档案与进度同步。Docker Compose 会自动读取 `.env.local`（如果存在）。也可通过 shell 环境变量或 `docker compose --env-file .env.local up --build` 传入。**未配置时不影响任何本地功能**。
